...
#         "FTA+Aug",
    ...
    groups = {
        "ReLU": ["ReLU", "ReLU+Control1g", "ReLU+Control5g", "ReLU+ATC", "ReLU+XY", "ReLU+Decoder", "ReLU+NAS", "ReLU+Reward", "ReLU+SF",],
#       "ReLU(L)": ["ReLU(L)", "ReLU(L)+Control1g", "ReLU(L)+Control5g", "ReLU(L)+XY", "ReLU(L)+Decoder", "ReLU(L)+NAS", "ReLU(L)+Reward", "ReLU(L)+SF",],
        "FTA": ["FTA eta=0.2", "FTA eta=0.4", "FTA eta=0.6", "FTA eta=0.8", "FTA+Control1g", "FTA+Control5g", "FTA+ATC", "FTA+XY", "FTA+Decoder", "FTA+NAS", "FTA+Reward", "FTA+SF",],
    }
...
#    property_scatter(property_keys, label_filter(targets, gh_nonlinear_transfer_sweep_v13_largeReLU), groups, "nonlinear/group-relu-atc-activation")
    groups = {
        "FTA": ["FTA eta=0.2"],
        "FTA+ATC": ["FTA+ATC"]
    }
    ...
    groups = {
        "ReLU": ["ReLU"],
        "ReLU+Ortho": ["ReLU+Ortho"],
        "ReLU(L)+Ortho": ["ReLU(L)+Ortho"],
    #    "ReLU+ATC": ["ReLU+ATC"]
    } 
    groups = {
        "FTA": ["FTA eta=0.2"],
    ...
    groups = {
       ...
       "ReLU+Laplacian": ["ReLU+Laplacian"],
        #  "ReLU+SF": ["ReLU+SF"],       
       "ReLU+Laplacian (prop)": ["ReLU+Laplacian (prop)"],
    #    "ReLU+ATC": ["ReLU+ATC"]
    }
    property_scatter(property_keys, label_filter(targets, gh_nonlinear_transfer_sweep_v13_largeReLU), groups, "nonlinear/group-laplacian-activation")
    ...
    groups = {
       ...
       "ReLU+Ortho": ["ReLU+Ortho"],
        #  "ReLU+SF": ["ReLU+SF"],       
       "ReLU+Ortho (prop)": ["ReLU+Ortho (prop)"],
    #    "ReLU+ATC": ["ReLU+ATC"]
    }

    property_scatter(property_keys, label_filter(targets, gh_nonlinear_transfer_sweep_v13_largeReLU), groups, "nonlinear/group-ortho-activation")
    ...
    #     "Reward": ["ReLU+Reward", "ReLU(L)+Reward", "FTA+Reward"],
    #     "SF":["ReLU+SF", "ReLU(L)+SF", "FTA+SF"],
    #     "ATC":["ReLU+ATC", "FTA+ATC"],
    # }
    # property_scatter(property_keys, label_filter(targets, gh_nonlinear_transfer_sweep_v13_largeReLU), groups, "nonlinear/group-auxiliary")
