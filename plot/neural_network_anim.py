...
def main(left, right, bottom, top, layer_sizes):
    ...
    CNN_xy = (0.40, 0.5-0.3350000000000001/2)
    CNN_width= 0.2
    ...
    radar_ax = fig.add_axes([0.42, 0.05, 0.22, .22], projection='radar')
    ...
    val_layer_xy = (representation_layer_xy[0] + representation_layer.width + NN_distance, 0.55)
    val_layer= Layer(val_layer_xy, 5)
    val_layer_xy = (representation_layer_xy[0] + representation_layer.width + NN_distance, representation_layer_xy[1]+(representation_layer.height - val_layer.height)/2)
    val_layer= Layer(val_layer_xy, 5)
    ...
    lines = flat_rep_connections.lines + rep_value_connections.lines + val_output_connections.lines #+ rep_aux_connections.lines 

    action_list = np.load('plot/anim_data/agent_action_list.npy')
    agent_pos_list = np.load('plot/anim_data/agent_pos_list.npy')
    steps = np.load('plot/anim_data/step_num_list.npy')
    episodes = np.load('plot/anim_data/episode_num.npy')
    properties = np.load('plot/anim_data/properties.npy')
    ...
    anim.save('neural_network.mp4', writer = 'ffmpeg', fps = 10)
